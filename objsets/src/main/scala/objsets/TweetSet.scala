...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val mostRetweetedLeft = if (!left.isEmpty) left.mostRetweeted else elem
    val mostRetweetedRight = if (!right.isEmpty) right.mostRetweeted else elem
    val mostRetweetedChild = if (mostRetweetedLeft.retweets > mostRetweetedRight.retweets) mostRetweetedLeft else mostRetweetedRight
