 ...
 * `Lists` that need to be implemesnted as part of this assignment. A test
 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of elements in empty list") {
    assert(sum(List()) === 0)
  }

  test("sum of elements in list with one element") {
    assert(sum(List(1)) === 1)
  }
  ...
  test("max element in list with repeated values") {
